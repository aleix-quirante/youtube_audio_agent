...
def download_audio(video_url: str):
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        ...
        audio_path = f"data/raw_audio/{title}.mp3"
...
def run_demucs(audio_path: str, title: str):
    ...
    subprocess.run(["demucs", audio_path, "-o", "data/separated"], check=True)

    # Demucs removes the .mp3 extension for the folder name
    base_name = os.path.splitext(os.path.basename(audio_path))[0]
    drums_path = f"data/separated/htdemucs/{base_name}/drums.wav"
...
def process_audio_to_json(audio_path: str, drums_path: str, title: str):
    ...
    min_frames = int(1.5 * sr / hop_length)
    ...
    json_path = f"data/agent_db/{title.replace(' ', '_')}_master.json"
